...
const Servicing = () => {
    ...
    const getOrders = async () => {
        const response = await fetch(`http://localhost:8000/api/auth/getservicable`, {
            method: "GET",
            headers: {
        ...
        <div className='flex justify-center my-16'>
            <div className='max-w-[1700px] flex flex-col w-full'>
                {items.length > 0 && <div className='flex sm:block px-3 w-full mt-11 md:mt-4 -space-x-8'>
                    <input type="search" name="orders" id="orders" placeholder='Search products that are in servicing' className='text-sm rounded-sm w-11/12 max-w-[1400px] m-auto outline-none focus:border-[#2874f0] border-[1px] border-gray-300 px-4 py-2 pr-11' value={searchVal} onChange={onChange} />
